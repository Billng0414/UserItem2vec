...
class Batcher(object):
    def __init__(self, input_file, u_interaction_file, u_tag_file, i_interaction_file, i_tag_file, usize=904807, isize=393561,
        ...
        randomWalk = RandomWalk(u_interaction_file, u_tag_file, i_interaction_file, i_tag_file, self.datas,
                                usize=usize, isize=isize,negative_size=self.negative_size, prob=self.prob)

        self.users, self.items, self.labels, self.steps, self.unode_samples, self.unei_samples, self.ulabels, \
                                self.inode_samples, self.inei_samples, self.ilabels = randomWalk.get_samples()

        self.data_size = len(self.users)
    ...
    def read_files(self):
        data = np.loadtxt(self.input_file, dtype=int)
        '''
        users = data[:, 0]
        items = data[:, 1]
        #labels = data[:, 2]
